...
fun Route.getOfferReceived(log: Logger) = authenticate(JWT_AUTH) {
    post("/offer/received") {
        call.principal<JWTPrincipal>()?.run {
            val user = findUserByCredential(payload.getClaim(JWT_USERNAME_CLAIM).asString())
            log.info("User Found:${user?.id}")
            try {
                val (offerReceiverId, productId) = call.receive<GetOfferReceivedRequest>()

                if (offerReceiverId <= 0 || productId <= 0) { //illogical case.
